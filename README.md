...
The function should take an optional second argument named `use_salts`. If set to true, each salt string from the file `known-salts.txt` should check whether the salt is prepended **OR** appended to each password from `top-10000-passwords.txt` before hashing, and then comparing it to the hash passed into the function.
